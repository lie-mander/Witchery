...
private:
    ...
    void CalculateNumOfHits(const FVector& TraceStart, const FVector_NetQuantize& HitTarget, TMap<AWTRCharacter*, uint32>& HitsMap,
        TMap<AWTRCharacter*, uint32>& HeadShotsHitsMap, FHitResult& FireHit);

    void HandleShotgunEffects(const FHitResult& FireHit);
    void ApplyDamageWithoutSSR(TMap<AWTRCharacter*, float>& DamageMap, AController* InstigatorController);
